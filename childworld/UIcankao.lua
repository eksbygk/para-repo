...
function worldmap.OnClickLevel(index)
    ...
    game.config.isVipOrHasPermission(function(permit)
        ...
        local isInstitutionUser = false
        local isOrgUser = KeepWorkItemManager.IsOrgVip()
        GameLogic.GetFilters():apply_filters('service.keepwork_service_project.is_project_read_only', 19405,
            function(isReadOnly, code)
                -- echo("hahaka",code)
                if (code == 1) then
