...
elderItemTable[1] = {filename = hat, text = "帽子"}
elderItemTable[2] = {filename = sunFlower, text = "向日葵"}
elderItemTable[3] = {filename = wine, text = "红酒"}
elderItemTable[4] = {filename = cake, text = "蛋糕"}
...
registerBroadcastEvent("section", function(msg)
    ...
    if section == 2 then
        -- 在末尾插入。章节二，加入向日葵物品
        table.insert(elderItemTable, {filename = sunFlower, text = "向日葵"})
    elseif section == 3 then
        -- 章节三，加入红酒，蛋糕
        table.insert(elderItemTable, {filename = wine, text = "红酒"})
        table.insert(elderItemTable, {filename = cake, text = "蛋糕"})
    end
end)
...
function init(entity)
    ...
    html =
        [[<div style="font-size: 28px; width: 1834px;height: 337px;background: url(images/dialog6.png);">]] ..
            elderItemTable[currentIndex].text .. [[</div>]]
end
init()
    ...
    if (entity) then
        if (msg.mountname == "" and entity:HasCustomGeosets()) then
            ...
            if (mountedEntity) then
                ...
                local pos = {
                    math.max(math.sqrt(dx ^ 2, dz ^ 2) - 0.2, 0), -0.1,
...
registerBroadcastEvent("onClickElder", function(msg) elderDialog() end)
